...
double get_cat021_item130_LAT(const cat021_item130 * item)
{
    ...
    return (double) (lat_raw * CAT021_ITEM130_LSB_LAT);
}
...
double get_cat021_item130_LON(const cat021_item130 * item)
{
    ...
    return (double) (lon_raw * CAT021_ITEM130_LSB_LON);
}
...
void set_cat021_item130_LAT(cat021_item130 * item, const double lat)
{
    uint32_t lat_raw;
    ...
    if (lat > 0.0)
        lat_raw = (uint32_t) (lat / CAT021_ITEM130_LSB_LAT + 0.5);
...
void set_cat021_item130_LON(cat021_item130 * item, const double lon)
{
    uint32_t lon_raw;
    ...
    // Change from degrees to raw value, rounding to nearest unit
    if (lon > 0.0)
        lon_raw = (uint32_t) (lon / CAT021_ITEM130_LSB_LAT + 0.5);
