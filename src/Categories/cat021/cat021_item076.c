...
{
    return (double) get_cat021_item076_TMRV_HP_raw(item) * CAT021_ITEM076_LSB_TMRV_HP;
}
...
void set_cat021_item076_FSI(cat021_item076 * item, const uint8_t raw_value)
{
    // TODO: Check value is in valid range
    SET_BITS(&(item->raw[0]), raw_value, MASK_02_BITS, 7);
}

void set_cat021_item076_TMRV_HP_raw(cat021_item076 * item, const uint32_t raw_value)
{
    // TODO: Check value is in valid range
...
void set_cat021_item076_TMRV_HP_seconds(cat021_item076 * item, const double seconds)
{
    ...
    if (seconds > 0)
        raw_value = (uint32_t) (seconds / CAT021_ITEM076_LSB_TMRV_HP + 0.5);
