...
ASTERIX_API uint16_t get_cat021_item148_ALT(const cat021_item148 * item)
{
    return GET_BITS((item)->raw, 1, MASK_13_BITS);
}
...
ASTERIX_API void set_cat021_item148_ALT(cat021_item148 * item, double alt)
{
    uint16_t alt_raw = 0;

    // TODO: Check value is within valid range
    if (alt > 0)
        alt_raw = (double) (alt * LSB_CAT021_ITEM148_ALT + 0.5);
