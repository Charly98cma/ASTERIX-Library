...
double get_I021_074_TMRP_HP_seconds(const I021_074 * item) {
 ...
 ******************************************************************************/
 ...
 void set_I021_074_FSI(I021_074 * item, const uint8_t raw_value) {
    // TODO: Check value is in valid range
    SET_BITS(&(item->raw[0]), raw_value, MASK_02_BITS, 7);
}

void set_I021_074_TMRP_HP_raw(I021_074 * item, const uint32_t raw_value) {
    // TODO: Check value is in valid range
...
void set_I021_074_TMRP_HP_seconds(I021_074 * item, const double seconds) {
    ...
    if (seconds > 0)
        raw_value = (uint32_t) (seconds / I021_074_LSB_TMRP_HP + 0.5);

    set_I021_074_TMRP_HP_raw(item, raw_value);
}
...
void print_I021_074(const I021_074 * item) {
    ...
    printf("  TMRP_HP (seconds) = %f\n\n", get_I021_074_TMRP_HP_seconds(item));
}
