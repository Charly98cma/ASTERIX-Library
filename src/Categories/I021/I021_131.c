...
{
    if (!item) return 0.0F;
    return read_twocomp_32bit(&item->raw[0]) * I021_131_LSB_LAT;
}

double get_I021_131_LON(const I021_131 * item)
{
    if (!item) return 0.0F;
    return read_twocomp_32bit(&item->raw[4]) * I021_131_LSB_LON;
}
...
void set_I021_131_LAT(I021_131 * item, double lat)
{
    ...
    write_twocomp_32bit(&item->raw[0], lat_raw);
}
...
void set_I021_131_LON(I021_131 * item, double lon)
{
    ...
    write_twocomp_32bit(&item->raw[4], lon_raw);
}
...
{
    printf("I021/131 - Position in WGS-84 Co-ordinates High-precision\n");
    if (!item)
    {
